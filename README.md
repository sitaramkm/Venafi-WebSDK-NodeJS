...
This is a sample template for Venafi-WebSDK-NodeJS - Below is a brief explanation of what we have generated for you:
...
│   └── package.json            <-- NodeJS depende1ncies and scripts
│   └── tests                   <-- Unit tests
│       └── unit
│           └── test-handler.js
...
```bash
sam logs -n HelloWorldFunction --stack-name venafi-websdk --tail
```
...
```bash
aws cloudformation delete-stack --stack-name venafi-websdk
```
